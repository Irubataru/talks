...
\colorlet{codeblockcolor}{OliveGreen!80!Black}

\setbeameroption{show notes}

\usetikzlibrary{calc,intersections,positioning,matrix,chains,scopes,arrows.meta}
...
\lstset{
  ...
  breaklines=true,
  % prebreak=\mbox{{\color{Bittersweet}\tiny\ $\hookleftarrow$}}, 
  % postbreak=\mbox{{\color{Bittersweet}\tiny$\to$\ }},
  tabsize=5,
  morekeywords={%
...
\begin{frame}[fragile]
  ...
  \begin{tikzpicture}[overlay, remember picture]
    \node[scale=0.6, anchor=east] at ([shift={(-2.5cm, 0.77cm)}] current page.east) (text) {What happens here?};
    \draw[-{Stealth[scale=0.8]}] (text.west) -- +(-1.2cm,0);
  \end{tikzpicture}
...
\begin{frame}
  ...
  \begin{center}
    \begin{tikzpicture}[scale=.6, every node/.style={scale=.6}]
      ...
      \node[uml-class, inner sep=7, align=left] (drawing) {\textbf{Drawing} \\[12pt] Shapes};
      \draw (drawing.west) -- (drawing.east);


      \only<1>{
        \node[uml-class] [below=of drawing] (shape) {\textbf{Shape} };
        \draw[{Diamond}-{Stealth}] (drawing.south) -- (shape.north);
      }

      \only<2>{
        \node[uml-class] [below=of drawing] (unique-ptr) { \textbf{std::unique\_ptr} \\[12pt] shape\_ptr };
        \draw (unique-ptr.west) -- (unique-ptr.east);
        \draw[{Diamond}-{Stealth}] (drawing.south) -- ( unique-ptr.north);
        \node[uml-class] [below=of unique-ptr] (shape) {\textbf{Shape} };
        \draw[{Diamond}-{Stealth}] (unique-ptr.south) -- (shape.north);
      }
...
\begin{frame}
  ...
  \begin{center}
    \begin{tikzpicture}[scale=.6, every node/.style={scale=.6}]
      ...
      \node[uml-class] [right=2cm of shape] (brush) {\textbf{Brush} };

      \only<1>{
        \draw[{Diamond[open]}-{Stealth}] (shape.east) -- (brush.west);
      }

      \only<2->{
        \node[uml-class] [above=of brush] (brush-ptr) { \textbf{std::shared\_ptr} \\[12pt] brush\_ptr };
        \draw (brush-ptr.west) -- (brush-ptr.east);

        \draw[{Diamond[open]}-{Stealth}] (shape.east) -- +(1.5cm, 0) |- (brush-ptr.west);
        \draw[{Diamond[open]}-{Stealth}] (brush-ptr.south) -- (brush.north);
      }
      
      \only<3->{
        \node[uml-class] [above=of brush-ptr] (drawing-tool) { \textbf{Drawing tool} \\[12pt] brush };
        \draw (drawing-tool.west) -- (drawing-tool.east);
        \draw[{Diamond[open]}-{Stealth}] (drawing-tool.south) -- (brush-ptr.north);
      }
...
std::shared_ptr<int[]> ap(new int[10]);(*\tikzmark{specdeleter}*)
  ...
  \begin{tikzpicture}[overlay, remember picture]
    ...
    \node[scale=0.7, anchor=east] at ([shift={(-1.5cm, -0.74cm)}] current page.east) (text-1) 
    {
      \begin{varwidth}{3cm}
        Destructs using \object{delete}
      \end{varwidth}
    };
    \draw[-{Stealth[scale=0.8]}] (text-1.west) -- +(-1.2cm,0);


    \node[scale=0.7, anchor=east] at ([shift={(-1.5cm, -1.5cm)}] current page.east) (text-2) 
    {
      \begin{varwidth}{3cm}
        Destructs using \lstinline!delete[]!
      \end{varwidth}
    };
    \draw[-{Stealth[scale=0.8]}] (text-2.west) -- +(-1.5cm,0);

    \node[scale=0.7, anchor=east] at ([shift={(-1.5cm, -2.42cm)}] current page.east) (text-3) 
    {
      \begin{varwidth}{3cm}
        Destructs using \lstinline!delete[]!
      \end{varwidth}
    };
    \draw[-{Stealth[scale=0.8]}] (text-3.west) -- +(-0.8cm,0);
...
\begin{frame}[fragile]
  ...
  \begin{center}
    \begin{tikzpicture}[scale=.6, every node/.style={scale=.6}]
      ...
      \draw ([yshift=8pt]base.west) -- ([yshift=8pt]base.east);

      \node[uml-class] [below=of base] (derived) {
          \textbf{Derived} \\[12pt]
          \texttt{void print();} \\
          \texttt{void sum();}
      };
      \draw ([yshift=8pt]derived.west) -- ([yshift=8pt]derived.east);

      \draw[-{Latex[open,scale=1.3]}] (base.south) -- (derived.north);
